...
ui <- dashboardPage(
  ...
  dashboardSidebar(
    sidebarMenu(
      ...
      menuItem("Dataset Tools", icon = icon("table"),
        menuSubItem("Import Dataset", tabName = "i", icon = icon("arrow-circle-up")),
        menuSubItem("Edit Dataset", tabName = "e", icon = icon("pencil-ruler")),
        menuSubItem("Filter Dataset", tabName = "f", icon = icon("filter"))
      ),
      menuItem("Visualization Tools", icon = icon("chart-line"),
        menuSubItem("Plots", tabName = "p", icon = icon("chart-area")),
        menuSubItem("Markov Matrices", tabName = "mm", icon = icon("border-none"))
      )
    )
  ),
    ...
    tabItems(
      tabItem(tabName = "hp", 
              column(width = 12,
                ...
                h3("Note: This application may appear odd on some computers with respect to 
                   orientation; monitors and such may have a different resolution than 
                   computersand thus may disorient some of the graphics. As these are 
                   discovered, user-defined settings will be included to make the UI more 
                   user-friendly."),
      ...
      tabItem(tabName = "i", 
              column(width = 6, 
                     ...
                     checkboxInput("trackman", "Is Trackman Data?", TRUE),
                       ...
                       fluidRow(
                         column(3,
                            selectInput("date_var", "Select \"Date\" Variable: ",
                                   choices = NULL),
                            selectInput("events_var", "Select \"Events\" Variable (e.g., 
                                   Single, Double, Groundout, etc.): ", choices = NULL),
                            selectInput("description_var", "Select \"Description\" Variable 
                                   (e.g., swinging-strike, hit, etc.): ", choices = NULL),
                         ...
                         column(3,
                           ...
                           selectInput("home_team_var", "Select \"Home Team\" Variable: ",
                                       choices = NULL),
                           selectInput("launch_speed_var", "Select \"Launch Speed (Exit Velo)\"
                                       Variable", choices = NULL)
                         )
                       )
                ),
                tabPanel("Actions",
                       ...
                       actionButton("swing_matrix_create", "Create Contact & Swing Matrix 
                                    Vars", icon = icon("bolt")),
                       textInput("exp_filename", "Filename to Save: ", placeholder = 
                                   "export1.csv"),
                       actionButton("edited_table_save", "Export Table to .csv", icon = 
                                    icon("share-alt"))
                ),
                tabPanel("View Edited Dataset",
      ...
      tabItem(tabName = "f", 
              ...
              h2("\n"),
              tabsetPanel(
                tabPanel("Select Filter Variables",
                      h3("Hard/Soft Contact & Swing-With-Miss"),
                      selectInput("c_swm_filter_var", "Select \"Hard/Soft Contact & 
                                  Miss\" Variable:",choices = NULL),
                      h3("Pitcher Name"),
                      selectInput("pn_filter_var", "Select Pitcher (if applicable)", 
                                  choices = NULL, multiple = FALSE)
                         ),
                tabPanel("Edit Filter Levels",
      ...
      tabItem(tabName = "p", 
              ...
              fluidRow(
                column(3,
                       h3("Use Filters?"),
                       checkboxInput("pt_filter_bool", "Use Pitch-Type Filter?", 
                                     value = FALSE),
                       checkboxInput("c_swm_filter_bool", "Use Contact Filter?", 
                                     value = FALSE),
                       checkboxInput("pn_filter_bool", "Use Pitcher-Name Filter?", 
                                     value = TRUE),
                ),
                column(3,
      ...
      tabItem(tabName = "mm",
              ...
              sidebarLayout(
                sidebarPanel = sidebarPanel(
                  tabsetPanel(
                    tabPanel(
                      "Variables",
                      h4("Decide what variable represents the foundation of the model (i.e. 
                      what is shifting and changing from observation to observation?"),
             ...
             actionButton("markov_make", "Create Markov Matrix", icon = icon("magic"))
                    ),
             
             tabPanel(
               "Descriptors",
               h3("\n"),
               h3("If these are changed, click 'Update Descriptors' to change them in the 
                  output."),
               ...
               textInput("markov_outfile", "Filename to Save (.html): ", value = 
                           "myMarkov1.html"),
               actionButton("desc_update", "Update Descriptors", icon = icon("comment-alt")),
...
server <- function(input, output, session) {
  ...
  values <- reactiveValues(table1 = NULL,
                           ...
                           markov = NULL,
                           markov_title = NULL,
                           markov_subtitle = NULL,
                           markov_caption = NULL,
                           markov_outfile = NULL)
  
  observeEvent(input$trackman, {
    if (input$trackman) {
      values$presetChoice = "tagTM"
    }
    else {
      values$presetChoice = "baseballr"
    }
  })
  ...
  data <- reactive({
    ...
    idxs = presetHandler(if1$datapath, "./presets.txt", values$presetChoice)$idxs
    
    updateSelectInput(session, "pt_filter_var", choices = names(df), selected = names(df)[idxs[4]])
    updateSelectInput(session, "c_swm_filter_var", choices = names(df)) 
    updateSelectInput(session, "pn_filter_var", choices = names(df)) 
    updateSelectInput(session, "plot_x", choices = names(df))
    updateSelectInput(session, "plot_y", choices = names(df))
    
    updateSelectInput(session, "date_var", choices = names(df), selected = names(df)[idxs[1]])
    updateSelectInput(session, "home_team_var", choices = names(df), selected = 
                        names(df)[idxs[6]])    
    updateSelectInput(session, "away_team_var", choices = names(df), selected = 
                        names(df)[idxs[5]])
    updateSelectInput(session, "events_var", choices = names(df), selected = names(df)[idxs[2]])
    updateSelectInput(session, "description_var", choices = names(df), selected = 
                        names(df)[idxs[3]])
    updateSelectInput(session, "launch_speed_var", choices = names(df), selected = 
                        names(df)[idxs[7]])
  ...
  observeEvent(input$id_create, {
    ...
    updateSelectInput(session, "markov_states_var", choices = names(values$df))
    updateSelectInput(session, "markov_divider_var", choices = names(values$df))
  })
  ...
  observeEvent(input$edited_table_save, {
    write.csv(values$df, input$exp_filename, row.names = FALSE)
  })
  ...
  output$pn_filter <- renderUI({
    if (is.null(values$df)) {
      values$df = data()
    }
    pitchers <- unique(values$df[,colnames(values$df) == input$pn_filter_var])
    checkboxGroupInput("pn_filter", "Select Pitcher(s): ", choices = pitchers, selected = pitchers[1])
  })
  ...
  output$plot1 <- renderPlot({
    ...
    if (input$c_swm_filter_bool) {
      df = df %>% filter(df[,colnames(values$df) == input$c_swm_filter_var] %in% 
                           input$c_swm_filter)
    }
    if (input$pn_filter_bool) {
      df = values$df %>% filter(values$df[,colnames(values$df) == input$pn_filter_var] %in% 
                                         input$pn_filter)
    }
  ...
  observeEvent(input$desc_update, {
    values$markov_title = input$markov_title
    values$markov_subtitle = input$markov_subtitle
    values$markov_caption = input$markov_caption
    values$markov_outfile = input$markov_outfile
  })
  
  observeEvent(input$markov_make, {
        df = NULL
    ...
    if (input$pt_filter_bool) {
      df = values$df %>% filter(values$df[,colnames(values$df) == input$pt_filter_var] %in% 
                                         input$pt_filter)
    }
    
    if (input$c_swm_filter_bool) {
      df = values$df %>% filter(values$df[,colnames(values$df) == input$c_swm_filter_var] %in% 
                                         input$c_swm_filter)
    }
    ...
    stateVar <- df[,colnames(df) == input$markov_states_var]
    dividerVar <- df[,colnames(df) == input$markov_divider_var]
    ...
    m1 <- markovTransitionMatrix(values$df, statesVar = stateVar, dividerVar = dividerVar, 
                                 precision = 3,
                                 titleString = values$markov_title, 
                                 subtitleString = values$markov_subtitle, 
                                 captionString = values$markov_caption, 
                                 filenameToSave = values$markov_outfile)
  ...
  observeEvent(input$markov_save, {
    fn = ""
    if (".html" %in% values$markov_outfile) {
      fn = values$markov_outfile
    }
    else {
      fn = paste(values$markov_outfile, ".html", sep = "")
    }
    if (is.null(values$markov)) {
      return()
    }
    saveMatrix(values$markov, values$markov_outfile)
  })
